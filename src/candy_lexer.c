...
/**
  ...
  *         if so, process this byte with the callback
  * @param  self  lexer
  * @param  check check function
  * @param  cb    char processing callback
  * @retval boolean
  */
static inline bool _check_next(candy_lexer_t *self, bool (*check)(char), void (*cb)(candy_lexer_t *)) {
  return check(_view(self, 0)) ? cb(self), true : false;
}
...
/**
  ...
  * @retval boolean
  */
static inline bool _check_dual(candy_lexer_t *self, const char str[], void (*cb)(candy_lexer_t *)) {
  return (_view(self, 0) == str[0] || _view(self, 0) == str[1]) ? cb(self), true : false;
}
...
/**
  ...
  * @return none
  */
static void _handle_newline(candy_lexer_t *self, void (*cb)(candy_lexer_t *)) {
  cb(self);
  _check_dual(self, "\r\n", cb);
#ifdef CANDY_DEBUG_MODE
  self->dbg.line++;
...
static candy_tokens_t _get_number(candy_lexer_t *self, candy_wrap_t *wrap) {
  ...
  if (((_view(self, 0) == '0') ? _save(self), true : false) && _check_dual(self, "xX", _save)) {
    ...
    _save_char(self, '\0');
    candy_wrap_set_integer(wrap, (candy_integer_t)strtol(_buff(self), NULL, 16));
    return TK_INTEGER;
  }
  _save(self);
  while (1) {
    switch (_view(self, 0)) {
      ...
      default:
        ...
        _save_char(self, '\0');
        if (is_float) {
          candy_wrap_set_float(wrap, (candy_float_t)strtod(_buff(self), NULL));
          return TK_FLOAT;
        }
        else {
          candy_wrap_set_integer(wrap, (candy_integer_t)strtol(_buff(self), NULL, 10));
          return TK_INTEGER;
        }
    }
  }
...
static candy_tokens_t _get_string(candy_lexer_t *self, candy_wrap_t *wrap, const bool multiline) {
  ...
  _skipn(self, multiline ? 3 : 1);
  candy_wrap_set_string(wrap, _buff(self), self->w - head);
  return TK_STRING;
}
...
int candy_lexer_init(candy_lexer_t *self, candy_io_t *io, candy_reader_t reader, void *ud) {
  ...
  self->w = 0;
  self->reader(_buff(self) + CANDY_LEXER_EXTRA_SIZE, _size(self) - CANDY_LEXER_EXTRA_SIZE, self->ud);
  return 0;
}
