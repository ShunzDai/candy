...
static void _execute_cfunction(candy_vm_t *self, candy_callinfo_t *ci, const candy_wrap_t *fn, candy_state_t *co) {
  candy_logd(TAG, "into %s %s:%d", __FUNCTION__, __FILE__, __LINE__);
...
static void _execute_sclosure(candy_vm_t *self, candy_callinfo_t *ci, const candy_wrap_t *fn, candy_state_t *co) {
  candy_logd(TAG, "into %s %s:%d", __FUNCTION__, __FILE__, __LINE__);
...
static void _call(vmll_t *self) {
  ...
  switch (candy_wrap_type(fn)) {
    case CANDY_TYPE_CFUNC:
      _execute_cfunction(self->vm, ci, fn, self->co);
      break;
    case CANDY_TYPE_SCLOS:
      _execute_sclosure(self->vm, ci, fn, self->co);
      break;
    default:
      assert(0);
      break;
