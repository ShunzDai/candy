...
static inline void candy_vector_set_cell(candy_vector_t *self, size_t cell) {
  self->cell = cell;
}
...
static inline candy_err_t candy_vector_init(candy_vector_t *self, size_t cell) {
  candy_vector_set_capacity(self, 0);
  candy_vector_set_cell(self, cell);
  candy_vector_set_size(self, 0);
  candy_vector_set_data(self, NULL);
  return CANDY_OK;
}

static inline candy_err_t candy_vector_deinit(candy_vector_t *self, candy_memory_t *mem) {
  candy_vector_set_data(self, candy_memory_free(mem, candy_vector_data(self),
    candy_vector_cell(self) * candy_vector_capacity(self)
  ));
  candy_vector_set_size(self, 0);
  candy_vector_set_cell(self, 0);
  candy_vector_set_capacity(self, 0);
  return CANDY_OK;
}

static inline void candy_vector_reserve(candy_vector_t *self, candy_memory_t *mem, candy_excep_t *ctx, size_t capacity) {
...
static inline candy_err_t candy_vector_append(candy_vector_t *self, candy_memory_t *mem, candy_excep_t *ctx, const void *data, size_t size) {
  size_t cap = candy_vector_capacity(self);
  ...
  if (size && data) {
    memcpy((uint8_t *)candy_vector_data(self) + candy_vector_cell(self) * sz, data, candy_vector_cell(self) * size);
  }
  candy_vector_set_size(self, candy_vector_size(self) + size);
  return CANDY_OK;
}

static inline void candy_vector_resize(candy_vector_t *self, candy_memory_t *mem, candy_excep_t *ctx, size_t size) {
  size_t sz = candy_vector_size(self);
  if (sz < size)
