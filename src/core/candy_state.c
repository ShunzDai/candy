...
static void _dostream(void *arg) {
  ...
  candy_excep_t *ctx = &self->co->ctx;
  candy_object_t *out = NULL;
  candy_err_t err = candy_parse(vm->gc, ctx, self->reader, self->arg, &out);
...
candy_err_t candy_state_dostream(candy_state_t *self, candy_reader_t reader, void *arg) {
  ...
  candy_err_t err = candy_excep_try(&self->ctx, _dostream, &pack, (candy_object_t **)&out);
  if (err != CANDY_OK && out != NULL) {
...
candy_err_t candy_state_call(candy_state_t *self, int narg, int nres) {
  candy_array_t *out = NULL;
  candy_err_t err = candy_vm_call(&self->vm, &self->ctx, narg, nres, self, (candy_object_t **)&out);
  return err;
}
...
candy_err_t candy_state_setglobal(candy_state_t *self, const char name[], candy_cfunc_t entry) {
  ...
  const candy_array_t *s = candy_array_create_const(self->vm.gc, &self->ctx, CANDY_TYPE_CHAR, name, strlen(name));
  candy_wrap_set_object(&key, (candy_object_t *)s);
  candy_wrap_set_cfunc(&val, entry);
  err = candy_table_set(g, self->vm.gc, &self->ctx, &key, &val);
  return err;
}
...
candy_err_t candy_state_push_integer(candy_state_t *self, candy_integer_t val) {
  ...
  candy_err_t err = candy_vm_push(&self->vm, &self->ctx, &wrap, 1);
  return err;
}
...
candy_integer_t candy_state_to_integer(candy_state_t *self, int idx) {
  const candy_wrap_t *wrap = candy_vm_view(&self->vm, &self->ctx, idx);
...
candy_types_t candy_state_get_type(candy_state_t *self, int idx) {
  const candy_wrap_t *wrap = candy_vm_view(&self->vm, &self->ctx, idx);
  candy_types_t type = candy_wrap_type(wrap);
  return type;
}

int candy_state_fprint(candy_state_t *self, int idx, FILE *out) {
  const candy_wrap_t *wrap = candy_vm_view(&self->vm, &self->ctx, idx);
  return candy_wrap_fprint(wrap, self->vm.gc, out);
}
