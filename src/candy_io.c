...
void candy_io_write(candy_io_t *self, char ch) {
  ...
  if (self->size - self->w < CANDY_IO_DEFAULT_BUFFER_SIZE / 2) {
    char *buffer = (char *)calloc(self->size + CANDY_IO_DEFAULT_BUFFER_SIZE, sizeof(char));
    memcpy(buffer, self->buffer, self->size);
    self->reader(buffer + self->size, CANDY_IO_DEFAULT_BUFFER_SIZE, self->ud);
    free(self->buffer);
    self->buffer = buffer;
