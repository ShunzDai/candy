  ...
  while (candy_lexer_lookahead(&self->ls) != TK_EOS) {
    switch (candy_lexer_lookahead(&self->ls)) {
      ...
      default:
        ...
        printf("lookahead(&self->lex) %d line %zu col %zu\n", candy_lexer_lookahead(&self->ls), self->ls.dbg.line, self->ls.dbg.column);
        candy_lexer_next(&self->ls);
        break;
    }
  }
...
candy_object_t *candy_parse(candy_gc_t *gc, candy_reader_t reader, void *arg) {
  ...
  return (candy_object_t *)candy_sclosure_create(gc, fs.block);
}
