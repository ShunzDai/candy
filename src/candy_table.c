...
void candy_table_delete(candy_table_t *self, candy_gc_t *gc) {
...
int expand(candy_table_t *self, candy_gc_t *gc) {
  candy_table_t t;
  float rate = (float)self->vec.size / self->vec.cap;
  static float avg = 0, count = 0;
  ++count;
  avg = (avg * (count - 1) + rate) / count;
  printf("cap %lu size %lu rate %.4f avg %.4f \n", self->vec.cap, self->vec.size, rate, avg);
  ...
  for (candy_pair_t *pair = _head(self); pair <= _tail(self); ++pair) {
    if (candy_wrap_get_type(&pair->key) == TYPE_NULL)
      continue;
