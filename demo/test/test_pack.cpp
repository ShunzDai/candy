...
extern "C" candy_node_t candy_node_delete(candy_node_t node);

TEST(pack, none){
  candy_pack_t pack = candy_pack_none((char *)"none");
  EXPECT_EQ((uint64_t)*candy_pack_get_buffer(pack), 0);
  pack = (candy_pack_t)candy_node_delete((candy_node_t)pack);
...
TEST(pack, string){
  ...
  EXPECT_EQ(candy_node_get_size(pack), sizeof("hello world"));
  ...
  EXPECT_EQ(candy_node_get_size(pack), sizeof("bye world"));
  pack = (candy_pack_t)candy_node_delete((candy_node_t)pack);
  EXPECT_EQ((uint64_t)pack, (uint64_t)NULL);
}
...
TEST(pack, unsigned){
  ...
  EXPECT_EQ(candy_node_get_size(pack), 5);
  ...
  EXPECT_EQ(candy_node_get_size(pack), 4);
  pack = (candy_pack_t)candy_node_delete((candy_node_t)pack);
  EXPECT_EQ((uint64_t)pack, (uint64_t)NULL);
}
...
TEST(pack, object){
  ...
  candy_object_get_method(obj, candy_time33((char *)"print"))(obj);
  pack = (candy_pack_t)candy_node_delete((candy_node_t)pack);
  EXPECT_EQ((uint64_t)pack, (uint64_t)NULL);
}
...
#define depth 1024
  ...
  for (int i = 0; i < depth; i++){
    ...
    i != 0 ? candy_object_push_object(obj[i - 1], (char *)"", obj[i]) : 0;
  }
  candy_pack_t pack = candy_pack_object((char *)"", obj[0]);
  pack = (candy_pack_t)candy_node_delete((candy_node_t)pack);
  EXPECT_EQ((uint64_t)pack, (uint64_t)NULL);
#undef depth
}
